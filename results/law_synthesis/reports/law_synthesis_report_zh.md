...
- budget phase: interior absolute-leverage peak supported = False; residual-vs-static per-cost leverage falls from 0.0311 to 0.0243
...
- non-obvious action law: target top-5% actions hidden from every simple top-5% rank = 34.2%; target top-5% below structure-only top-20% = 83.9%
...
| surrogate_architecture                | V16 adds a factorized leave-one-city action-value surrogate with interaction ablation; it is still not a full graph neural model. | The low-dimensional activated structure is strongly supported, while unrestricted interaction terms are not necessary in the current evidence.               | Use an explicit graph or OD-message-passing surrogate only if the paper needs to test higher-order spatial representations. |
