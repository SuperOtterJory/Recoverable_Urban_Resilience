...
| surrogate_architecture                | Current surrogate is normalized ridge/ranking evidence rather than a full graph neural model.                  | The symbolic law is already interpretable, but the neural structure-extractor stage is still lightweight. | Train a factorized graph/action-value surrogate if the paper needs a stronger AI-law-discovery component.                   |
| perturbed_optimum_stability           | Action-value labels and residual policies are validated, but perturbed-optimum frequency is not yet generated. | Robustness to near-optimal degeneracy is only indirectly covered.                                         | Add cost/effectiveness perturbation solves for a small representative sample.                                               |
