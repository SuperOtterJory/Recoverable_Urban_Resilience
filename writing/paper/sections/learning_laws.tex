...
A broader explicit LP closure strengthens this interpretation beyond the six representative hybrid events. On 12 additional footprint-sensitive non-New-York city-events, all 36 LPs solve to optimality. The near-free \(\lambda=0.02\) point raises top-5 percent allocated footprint mass from 0.0583 to 0.0754 with only 0.0008 LP-gain loss, while \(\lambda=0.05\) raises mass to 0.1085 but lowers gain/LP to 0.9887. The city-structure conclusion is therefore not that observed footprint is itself the recovery objective; it is that OD-activated recovery value and observed footprint define a stable secondary-objective frontier.

A New York-scale boundary audit prevents the remaining gap from being misread. Among the five most footprint-sensitive New York events, hybrid finite-value top-5 percent footprint mass rises from 0.0527 to 0.4042 on average, so the unresolved case is not caused by absent event-footprint signal. The largest New York footprint LP contains 1,940 zones, about 475,300 variables, 550,973 constraints, and 9.20 million access-loss nonzero terms---roughly 2.2 times the largest V42 closed model by variables and 2.1 times by access terms. The previous direct hybrid-footprint LP attempt for New York event 117 ended at the solver time limit before returning a feasible optimized solution. We therefore treat New York-scale direct footprint closure as a computational boundary, while keeping the smaller full-zone frontier result as the current positive evidence.

A follow-up active-set screen shows why the New York case should not be reduced back to a small OD sample. Keeping all 1,940 zones, state variables, and OD access-loss constraints, a token-column screen based on small-signal value, finite value, observed footprint, and OD structure uses 25.1 percent of action tokens and reduces total variables by about 55.0 percent. It captures 89.8 percent of observed footprint mass and 82.7 percent of destination importance, but only 49.5 percent of small-signal value and 60.7 percent of finite value. Reaching near-complete small-signal, finite-value, and footprint capture across the selected New York events requires 83.3 percent of action tokens and leaves only 12.3 percent variable reduction. The remaining New York task is therefore a decomposition problem---column generation, active-set expansion, or warm starts over the full state/access system---rather than a justification for regional downsampling.
...
This non-obviousness is not only a base-parameter artifact. Repeating the same diagnostic under 11 eta, cost, delay, and channel-favored parameter scenarios gives mean false-positive shares of 31.8 percent for deficit-only ranking, 25.0 percent for exposure-only ranking, and 81.0 percent for structure-only ranking. On average, 39.9 percent of true top-5 percent actions remain hidden from all three simple top-5 percent rankings, while 84.9 percent fall below the structure-only top-20 percent. The activated score is used here as a first-order consistency reference rather than independent causal evidence, but the stable failure of one-factor rules supports the law's central mechanism: recoverability depends on joint activation, not on a single large deficit, flow, or structural-bottleneck feature.

An exact selected-action audit gives a useful boundary on this claim. Re-solving 20 representative parameter-scenario LPs and examining the optimizer's positive allocations, the activated top-20 percent rank contains 97.0 percent of LP-selected cost. The union of deficit-only, exposure-only, and structure-only top-20 percent ranks also covers 95.0 percent of selected cost, so finite-budget LP support is not invisible to simple deficit and exposure signals. The strong failure is instead single-factor sufficiency: structure-only top-20 percent captures only 22.3 percent of selected cost, and 77.7 percent of selected cost lies below the structure-only top-20 percent. Thus the non-obvious law should be read as a rejection of single-factor prioritization, especially static structural centrality alone, rather than as a claim that all optimized actions are hidden from every simple signal.
...
To test scenario-specific closure directly, representative non-base LP optima were re-solved for selected city-events. After longer resumed solves, 26 of 28 representative non-base closures solve to optimality. Across these successful closures, static small-signal greedy captures 0.7085 of the scenario LP gain on average, while residual finite greedy captures 0.9405. The residual policy improves over static greedy in 92.3 percent of successful closures. The two unresolved rows are New York event 106 under low and high budget on 1,940 zones after extended solve attempts, so this result should be read as strengthened representative scenario-optimum validation rather than full non-base closure over all 105 events.
...
A severity-decoupling audit makes this contrast concrete. Using rank thresholds, there are 13 high-loss but low-decision-critical events and 19 moderate-or-lower-loss but high-decision-critical events; 16 high-rain events also fall below the decision-criticality median. In leave-city event-level prediction, severity-only features have near-zero pooled Spearman correlation with decision-criticality (-0.0402), whereas top-tail structure features reach 0.6790. This event-level law is therefore not a rainfall-severity law. Its current boundary is equally important: the top-5 percent value share has a between-city variance share of 0.5990, and the footprint-identifiability audit shows zero within-city variation in top-5 percent value share for five of seven cities, covering 58.1 percent of events. The top-10 greedy-recovery unit sets also have high within-city pairwise overlap, with mean Jaccard 0.9685. A TMC-derived footprint proxy then shows that this boundary is not caused by absent spatial data: across all 105 events, raw TMC speed abnormalities mapped to OD zones have mean footprint-template cosine 0.3587, mean top-20 footprint/template Jaccard 0.0111, and the OD-template top 5 percent zones capture only 4.8 percent of observed footprint mass. A hybrid calibration sensitivity test on the 100 footprint-covered optimized events separates two effects. The current small-signal target is unchanged when the footprint is injected, with mean base--hybrid Spearman 1.0000 and top-5 percent action Jaccard 1.0000, because the label only records whether future positive loss remains active. By contrast, the magnitude-aware finite-value field changes substantially: mean finite-value Spearman falls to 0.9579, top-5 percent action Jaccard falls to 0.5403, and observed footprint mass captured by finite top-5 percent units rises from 0.0474 to 0.3167. Representative full hybrid LP solves give the next boundary: six of seven selected footprint-sensitive city-events solve to optimality, but the optimized support changes only slightly, with selected-action Jaccard 0.9228, selected-unit Jaccard 0.9371, and selected-unit footprint mass rising only from 0.1402 to 0.1470. A mechanism audit shows why: the first-order-to-LP transfer ratio is only 0.0266 on average. Total budget is fully used, but period-budget binding is rare (0.0556), cap saturation is absent, and only 0.0675 of selected cost reaches beyond the first diminishing-return segment. Footprint top-5 percent units have high finite-value rank (0.8776) but much lower small-signal rank (0.4924), whereas selected units have small-signal rank 0.9206. A footprint-aware score frontier then clarifies how this signal should enter the law: footprint-only ranking captures 0.4906 of observed footprint mass but only 0.3479 of hybrid LP selected cost, while a gated finite-by-small-signal rank score preserves 0.9909 selected-cost capture and raises top-5 percent unit footprint mass from 0.0429 to 0.2284. When the same scores are used as actual finite-budget replay policies, the distinction sharpens: weak footprint weighting (\texttt{small\_plus\_0p25\_footprint}) slightly improves replay gain relative to small-signal greedy, from 0.7694 to 0.7731 of hybrid LP gain, and raises top-5 percent unit footprint mass from 0.0402 to 0.0574; by contrast, \texttt{finite\_x\_small\_rank} raises footprint mass to 0.2124 but lowers replay gain to 0.6791, and footprint-only replay falls to 0.1922. Under adaptive residual replay, pure residual finite greedy already captures 0.9669 of hybrid LP gain with top-5 percent footprint mass 0.0480. A weak residual footprint correction raises footprint mass to 0.0765 with a small gain loss of 0.0026, while stronger footprint gating raises mass further but loses more recovery gain. A fine residual footprint-weight scan makes this boundary explicit: \(\lambda=0.02\) slightly increases both gain (0.9685 of hybrid LP gain) and footprint mass (0.0494), but allowing up to 0.01 gain loss selects \(\lambda=0.35\), raising footprint mass by 0.0335 while lowering gain by 0.0050; the maximum-footprint point reaches mass 0.1200 only by lowering gain to 0.8927. Solving the same preference directly inside the LP objective confirms the boundary more strongly. Across 42 explicit multi-objective LPs, \(\lambda=0.02\) raises top-5 percent allocated-unit footprint mass from 0.0482 to 0.0734 while reducing the true recovery-gain fraction by only 0.0004; a larger footprint weight reaches mass 0.2446, but the true recovery-gain fraction falls to 0.4170. Thus the present event-level LP signal is partly a city-structure signal induced by OD exposure and the current small-signal calibration, while the raw data contain event-specific spatial footprints that can weakly refine magnitude-aware recoverability policies only as a tiny tie-breaker or explicit multi-objective preference, not as a sufficient recovery-action target.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\linewidth]{../../results/residual_footprint_tradeoff/figures/residual_footprint_tradeoff_frontier.png}
  \caption{Explicit residual recovery--footprint trade-off. Very small footprint weights can act as tie-breakers, while larger footprint coverage lies on a frontier with recovery-gain loss.}
  \label{fig:residual-footprint-tradeoff}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\linewidth]{../../results/broader_multiobjective_footprint_lp_validation/figures/multiobjective_lp_frontier.png}
  \caption{Broader direct multi-objective LP recovery--footprint frontier. When observed footprint coverage is rewarded inside the LP objective, weak weights remain near-free tie-breakers across additional city-events, while larger footprint gains require true recovery-gain loss.}
  \label{fig:multiobjective-footprint-lp}
\end{figure}
