...
def main() -> None:
    ...
    parser.add_argument("--optimization-dir", default="results/large_scale_optimization")
    parser.add_argument("--output-dir", default="results/large_scale_city_structure")
    ...
    dataset.to_csv(table_dir / "large_scale_city_structure_dataset.csv", index=False)
    correlations.to_csv(table_dir / "large_scale_structure_correlations.csv", index=False)
    rankings.to_csv(table_dir / "large_scale_city_rankings.csv", index=False)
    make_figures(dataset, correlations, figure_dir)
    write_report(report_dir / "large_scale_city_structure_report_zh.md", dataset, correlations, rankings)
    print(f"Wrote large-scale city-structure analysis to {output_dir}")
...
def write_report(path: Path, dataset: pd.DataFrame, correlations: pd.DataFrame, rankings: pd.DataFrame) -> None:
