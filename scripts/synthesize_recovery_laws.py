    ...
    return pd.DataFrame(
        [
            ...
            },
            {
                ...
                "implication": "The data contain usable event-specific spatial signal. Footprint-only ranking is not a recovery-action law, but a footprint/finite-magnitude term can enter as a gated refinement when it is multiplied by small-signal OD activation.",
                "next_step": "If footprint-specific recovery law becomes a main claim, rerun broader hybrid LP closure and validate the gated footprint-aware law as a finite-budget policy rather than only a support-ranking frontier.",
            },
            {
...
def write_report(
    ...
    lines = [
        "# Recoverability Law Synthesis V37",
        "",
        "V37 adds a footprint-aware score frontier on top of the V36 mechanism audit. It tests whether observed event footprints can enter the action law as a gated refinement without sacrificing alignment with hybrid LP selected support.",
        "",
        "## 本版做了什么",
        "",
        "V37 evaluates 26 candidate scores on the representative hybrid LP events. Each score is judged on two axes: how much hybrid LP selected cost appears in the score's top-20% actions, and how much observed event footprint mass is covered by the score's top-5% units.",
        "",
        "## 当前可写入论文的 law",
        "",
